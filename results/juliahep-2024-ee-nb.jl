...
md"""# JuliaHEP 2024 Plots

Plots for Jet Reconstruction talk at JuliaHEP 2024
"""
...
let
    ...
    for (sn, slc) in enumerate(durham_backend_df)
...
durham_aos_df = all_aos_results_df[select_results_rows(all_aos_results_df,
                                                      Dict("algorithm" => "Durham",
													         "backend" => "Julia")
                                                           ), :];
...
# ╔═╡ 09230e2c-790b-40dd-9917-385f0a7ba24f
...
let
    ...
    ax = Axis(eekt_antikt_plot[1, 1],
              ...
              xlabel = "Average Cluster Density", ylabel = "μs per event",
			  limits = (nothing, 75, 0.0, nothing))
	colour_counter = Dict("FastJet" => 0, "Julia" => 0)
	for (i, (k, subdf)) in enumerate(pairs(eekt_antikt_gdf))
...
manifest_format = "2.0"
project_hash = "e59c55587170ed0c4a7492833cadccda5c3470fe"
